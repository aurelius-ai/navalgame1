...
public class GridHelper implements Serializable {
	...
	AssetManager man;
	Rand random;
	public GridHelper(long randomSeed,AssetManager eman) {
	 ...
	 */
	public static boolean canPlaceInGrid(AssetManager em,byte rotateto, int row, int col, int width) {
		boolean flag = true;
		if (rotateto == GridedEntityTileOrientation.ORIENTATION_LEFTTORIGHT) {
			for (int c = 0; c < width; c++) {
				...
				Tile<Asset> temp = em.getTile(row,col+c);
				if(temp!=null) {
					flag=false;
					break;
				}
		...
		if (rotateto == GridedEntityTileOrientation.ORIENTATION_TOPTOBOTTOM) {
			for (int c = 0; c < width; c++) {
				...
				Tile<Asset> temp = em.getTile(row-c,col);
				if(temp!=null) {
					flag=false;
					break;
				}
	...
	public static boolean canMoveTo(AssetManager em,MoveableEntity e,byte position, int row, int col, int width) {
	...
	public static boolean canAttackPrimaryTo(AssetManager em,MoveableEntity e, int row, int col) {
		if(!e.isInPrimaryRange(col,row)){
			return false;
		}
		Tile<Asset> temp = em.getTile(row,col);
		if((temp==null||temp.getEntity().equals(e))){
			return false;
		}
		return true;
	...
	public static boolean canAttackSecondaryTo(AssetManager em,MoveableEntity e, int row, int col) {
