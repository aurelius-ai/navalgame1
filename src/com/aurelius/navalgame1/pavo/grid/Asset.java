...
public class Asset implements Serializable {
	...
	public Asset(AssetManager em,Location loc, GridedEntityTileOrientation id,byte orientation) {
		manager = em;
		location = loc;
	...
	/**
	 * Initializes the entity. This should never be called. If inheriting <code>Asset></code>, this method should probably be overriden.
	 */
	public void init() {
		setWidth(1);
		...
		if (akamai == GridedEntityTileOrientation.ORIENTATION_LEFTTORIGHT) {
			if (ORIENTATION_BUFFER_POSITION == GridedEntityTileOrientation.ORIENTATION_TOPTOBOTTOM){//ORIENTATION_BUFFER_POSITION equals getCurrentOrientation()
				...
				for (int w = 0; w < getWidth(); w++) { //places the entity in the correct position
					Tile<Asset> t = new Tile<Asset>(this,location.getRow(),location.getCol());
	...
	public void setId(GridedEntityTileOrientation id) {
		...
		if (ORIENTATION_BUFFER_POSITION == GridedEntityTileOrientation.ORIENTATION_LEFTTORIGHT) {
			for (int w = 0; w < getWidth(); w++) {
				Tile<Asset> t22 = new Tile<Asset>(this,location.getRow(),location.getCol()+w);
			...
			for (int h = 0; h < getWidth(); h++) {
				Tile<Asset> t22 = new Tile<Asset>(this,location.getRow()-(getWidth()-1)+h,location.getCol());
	 ...
	 * @param r The row to move the entity to.
	 * @param c The column to move the entity to.
	 * @param override Should current assets at that location be overridden?
	...
	public boolean moveTo(Location loc, boolean override) {
		...
		isHide = false;
		Tile<Asset> t = manager.getTile(loc);
		...
		if (ORIENTATION_BUFFER_POSITION == GridedEntityTileOrientation.ORIENTATION_LEFTTORIGHT) {
			...
			for (int w = 0; w < getWidth(); w++) {
				Tile<Asset> t22 = new Tile<Asset>(this,loc.getRow(),loc.getCol()+w);
			...
			for (int h = 0; h < getHeight(); h++) {
				Tile<Asset> t22 = new Tile<Asset>(this,loc.getRow()+h-getHeight()+1,loc.getCol());
				t22.setId(new Id(id.memCall(ORIENTATION_BUFFER_POSITION)[0],h));
	...
	/**
	 * Gets the upper left tile of the Asset's location.
	 * @return The location. Could be "Unknown", if the Asset is not in the Grid.
	 */
	...
	public void onHit(Asset attackingEntity) {
		
	}
	
	public void onAttack(Asset entityBeingAttacked) {
