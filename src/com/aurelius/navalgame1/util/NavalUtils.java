...
import com.navalgame.achievement.Achievement;
import com.navalgame.leaderboard.Leaderboard;

public class NavalUtils {
		...
		if(!NavalBattle.getGameState().isOffline()) {
			if(!NavalBattle.getBroadcastService().needsUpdate()) {
				...
				NavalBattle.getWindowHandler().getToasterManager().showToaster(
						new ImageIcon(NavalUtils.class.getResource("/com/roketgamer/res/logo_100px.png")),
						"Submitted score of " + score + " to server");
				
				submitAchievement(NavalGameData.ACHIEVEMENT_SYNCING_SHIP);
			} else {
		...
		if(!NavalBattle.getGameState().isOffline()) {
			if(!NavalBattle.getBroadcastService().needsUpdate()) {
				if(!achievement.hasAchieved()) {
					...
					SoundUtils.playSound(NavalUtils.class.getResourceAsStream("/com/jpii/navalbattle/res/sfx/achievement.wav"));
