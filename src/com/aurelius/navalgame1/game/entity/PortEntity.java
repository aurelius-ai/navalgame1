...
public class PortEntity extends Animation {
	...
	public void spawnSubmarine(){
		...
		if(GridHelper.canPlaceInGrid(getManager(),GridedEntityTileOrientation.ORIENTATION_LEFTTORIGHT, to.getRow(), to.getCol(), 2))
			tm.addEntity(new Boat(getManager(), to, GridedEntityTileOrientation.ORIENTATION_LEFTTORIGHT),tm.findPlayer(this));
		else if(GridHelper.canPlaceInGrid(getManager(),GridedEntityTileOrientation.ORIENTATION_TOPTOBOTTOM, to.getRow(), to.getCol(), 2))
			tm.addEntity(new Boat(getManager(), to,GridedEntityTileOrientation.ORIENTATION_TOPTOBOTTOM),tm.findPlayer(this));
	}
	...
	public void spawnAC(){
		...
		if(GridHelper.canPlaceInGrid(getManager(),GridedEntityTileOrientation.ORIENTATION_LEFTTORIGHT, to.getRow(), to.getCol(), 5))
			tm.addEntity(new BirdFarm(getManager(), to, GridedEntityTileOrientation.ORIENTATION_LEFTTORIGHT),tm.findPlayer(this));
		else if(GridHelper.canPlaceInGrid(getManager(),GridedEntityTileOrientation.ORIENTATION_TOPTOBOTTOM, to.getRow(), to.getCol(), 5))
			tm.addEntity(new BirdFarm(getManager(), to,GridedEntityTileOrientation.ORIENTATION_TOPTOBOTTOM),tm.findPlayer(this));
	}	
	...
	public boolean takeDamage(int damage){
		...
		if(currentHealth <= 0){
			...
			repair();
			NavalUtils.submitAchievement(NavalGameData.ACHIEVEMENT_PORKCHOP);
		}
		return flag;
	}
