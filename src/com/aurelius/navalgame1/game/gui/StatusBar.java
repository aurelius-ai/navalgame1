...
import com.aurelius.navalgame1.pavo.gui.MessageBox;
import com.aurelius.navalgame1.pavo.gui.MessageBoxIcon;
...
public class StatusBar extends PWindow {
	...
	public void paint(Graphics2D g) {
		...
		if(NavalBattle.getGameState().isOffline())
			...
			g.drawImage(new ImageIcon(NavalBattle.getNavalGame().getPlayer().getAvatarAsBytes(16)).getImage(),width-348+sd,5,null);
			g.drawString(NavalBattle.getNavalGame().getPlayer().getName(), width-325+sd, 17);
		}
	}
	...
	public boolean mouseDown(MouseEvent me) {
		if(me.getX() >= width-326 && me.getX() <= (width-326) + 100) {
			if(NavalBattle.getGameState().isOffline())
				MessageBox.show("Offline Mode","Connect to the internet to enjoy NavalGame features.",
